...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_drink) { described_class.new('cocktail', 16) }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_drink.type).to eq('cocktail')
      end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:milk_drink) { described_class.new('milk', 2) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(milk_drink).to_not be_full
      end
    end
  end
end
